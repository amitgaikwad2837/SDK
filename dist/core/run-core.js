...
export function runCore(options) {
    try {
        ...
        stats.tablesCreated = Array.from(afterTableNames).filter(t => !beforeTableNames.has(t)).length;
        stats.tablesDropped = Array.from(beforeTableNames).filter(t => !afterTableNames.has(t)).length;
        stats.tablesModified = Array.from(afterTableNames).filter(t => beforeTableNames.has(t)).length;
        ...
        // Determine summary
        const totalChanges = stats.tablesCreated +
...
//# sourceMappingURL=run-core.js.map