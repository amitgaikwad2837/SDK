export function generateMigrations(before, after) {
    const migrations = [];
    const impacts = [];
    // Create lookup maps
        ...
        if (!afterTable) {
            ...
            if (columnChanges.added.length > 0) {
                for (const col of columnChanges.added) {
                    forwardStatements.push(`ALTER TABLE ${tableName} ADD COLUMN ${formatColumn(col)};`);
                    rollbackStatements.unshift(`ALTER TABLE ${tableName} DROP COLUMN ${col.name};`);
                }
            }
            if (columnChanges.removed.length > 0) {
                for (const col of columnChanges.removed) {
                    forwardStatements.push(`ALTER TABLE ${tableName} DROP COLUMN ${col.name};`);
                    rollbackStatements.unshift(`ALTER TABLE ${tableName} ADD COLUMN ${formatColumn(col)};`);
                    impacts.push({
                        type: "data_loss",
                        severity: "error",
                        table: tableName,
            ...
            if (columnChanges.modified.length > 0) {
                for (const { before: oldCol, after: newCol } of columnChanges.modified) {
                    ...
                    if (typeChanged || nullabilityChanged) {
                        forwardStatements.push(`ALTER TABLE ${tableName} MODIFY COLUMN ${formatColumn(newCol)};`);
                        rollbackStatements.unshift(`ALTER TABLE ${tableName} MODIFY COLUMN ${formatColumn(oldCol)};`);
                        if (typeChanged) {
                            impacts.push({
...
function getColumnChanges(before, after) {
    ...
    for (const [colName, afterCol] of afterMap) {
        ...
        else if (beforeCol.type !== afterCol.type ||
